...
struct EmojiArtDocumentView: View {
    ...
    private var documentBody: some View {
        GeometryReader { geometry in
            ZStack {
                Color.white
                if emojiArtDocument.background.isFetching {
                    ProgressView()
                        .scaleEffect(2)
                        .tint(.blue)
            ...
            .onTapGesture(count: 2) {
                zoomToFit(emojiArtDocument.bbox, in: geometry)
            }
            .dropDestination(for: Sturldata.self) { sturldatas, location in
                return drop(sturldatas, at: location, in: geometry)
            }
            .onChange(of: emojiArtDocument.background.failureReason) { oldReason, newReason in
                showBackgroundFailureAlert = (newReason != nil)
            }
            .onChange(of: emojiArtDocument.background.uiImage) { oldImage, newImage in
                zoomToFit(newImage?.size, in: geometry)
                
            }
            .alert("Set Background",
                   isPresented: $showBackgroundFailureAlert,
                   presenting: emojiArtDocument.background.failureReason,
                   actions: { reason in
    ...
    private func documentContents(in geometry: GeometryProxy) -> some View {
        if let uiImage = emojiArtDocument.background.uiImage {
    ...
    private func drop(_ sturldatas: [Sturldata], at location: CGPoint, in geometry: GeometryProxy) -> Bool {
        for sturldata in sturldatas {
            switch sturldata {
                case .url(let url):
                    emojiArtDocument.setBackgrond(url, undoWith: undoManager)
                    return true
